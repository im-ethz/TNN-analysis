...
# --------------------- find out timezone windows for each rider
...
# Reset counter n
df_tz.reset_index(inplace=True)
df_tz['n'] = df_tz.groupby('RIDER').cumcount()
df_tz = df_tz.set_index(['RIDER', 'n'])
...
for i in df_tz.index.get_level_values(0).unique():
	for n in df_tz.loc[i].index:
		# if first timezone or some gap due to censoring
		# (for 3,2 3,5, 6,57 and 15,7)
		if n == 0 or pd.isnull(df_tz.loc[(i,n-1), 'local_timestamp_final_diff']):
			df_tz.loc[(i,n), 'timezone_final'] = df_tz.loc[(i,n), 'timezone']
		else:
			df_tz.loc[(i,n), 'timezone_final'] = df_tz.loc[(i,n-1), 'timezone_final'] + df_tz.loc[(i,n-1), 'local_timestamp_final_diff']
